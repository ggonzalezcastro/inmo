...
- `generate_response_with_function_calling(system_prompt, contents, tools, tool_executor, tool_mode_override, ...)` — returns `(text, function_calls)` and triggers tool execution via the executor callback
- `build_llm_prompt(broker_id, lead_context, ...)` — assembles the full system prompt

**Function Calling & Tools:**
- `LLMToolDefinition` — unified tool format with name, description, parameters (JSON Schema)
- `tool_executor` — async callback invoked for each tool call. Agents wrap tool logic here and capture handoff intents.
- `tool_mode_override` — controls Gemini function calling mode (phase 3.1):
  - Passed to `GeminiProvider.generate_with_tools()` which uses it on the first iteration
  - `"ANY"` forces function calling; `"AUTO"` lets LLM decide; defaults to `"ANY"`
- Agentic loop (Gemini provider): calls LLM → extracts tool calls → executor runs them → feeds results back to LLM in contents → repeats up to 5 times
...
- Sticky: if `current_agent` is set, stay with it until agent signals handoff

**Handoff mechanism** (semantic, LLM-driven):
- Each agent has `_HANDOFF_TOOLS` — tool definitions passed to LLM
- LLM calls `handoff_to_<agent>` when it decides to transfer (tool calling)
- Tool executor captures intent in `_handoff_intent` dict → builds `HandoffSignal`
- `tool_mode_override` controls LLM behavior:
  - `"ANY"` — force function calling (used by agents with mandatory tool actions)
  - `"AUTO"` — let LLM decide (used by agents with optional handoff tools)

**Why tool-based?** Replaces fragile keyword lists (`["comprar", "casa", "departamento"]`) with semantic understanding. LLM reads natural language and calls the right handoff tool.

`AgentContext` is an immutable snapshot; `HandoffSignal` carries context updates across agent boundaries.
...
1. **Type hint mismatch in `gemini_provider.py` line 178**
   ...
   - Facade handles this with `len(result) >= 3` check, but hint is misleading
...
2. **Redundant variable initialization in `scheduler.py` line 163**
   - `tools: list = list(_HANDOFF_TOOLS)` is immediately overwritten on line 178
...
- **Added:** tool-based handoffs; agents define `_HANDOFF_TOOLS` and let LLM decide when to call them
- **Changed:** `AgentSupervisor._select_agent()` now uses deterministic `_STAGE_TO_AGENT` table instead of polling `should_handle()`
- **Deprecated:** `BaseAgent.should_handle()` no longer abstract; still exist for backward compat but supervisor doesn't call them
