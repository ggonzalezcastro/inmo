"""Facebook Page Messenger provider backed by Meta Graph API."""
...
class MessengerProvider(BaseChatProvider):
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.asset_id = str(config.get("asset_id") or "")
        self.access_token = config.get("access_token")
        if not self.asset_id or not self.access_token:
            raise ValueError("Messenger asset_id and access_token are required")
    ...
    async def parse_webhook_message(self, payload: Dict[str, Any]) -> Optional[ChatMessageData]:
        try:
            ...
            return ChatMessageData(
                ...
                channel_message_id=message.get("mid") or postback.get("mid"),
                message_text=text,
                direction="in",
